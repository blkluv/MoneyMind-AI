...
import { getChatInsight } from '../geminiService';
...
const MoneyCoachModal: React.FC<MoneyCoachModalProps> = ({ isOpen, onClose, transactions }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-40 md:p-4 backdrop-blur-sm">
      <div className="bg-background dark:bg-dark-background w-full h-full md:h-[80vh] md:max-w-2xl flex flex-col p-4 md:rounded-2xl shadow-xl transition-all">
        <div className="flex justify-between items-center border-b border-border-color dark:border-dark-border-color pb-3 mb-4">
            ...
            <button onClick={onClose} className="p-2 text-text-secondary dark:text-dark-text-secondary hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full">
        ...
        <div className="flex-grow overflow-y-auto pr-2 space-y-4">
            {messages.map((msg, index) => (
                <div key={index} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] md:max-w-md p-3 rounded-lg ${msg.sender === 'user' ? 'bg-primary text-white' : 'bg-card-bg dark:bg-dark-card-bg text-text-primary dark:text-dark-text-primary shadow-sm'}`}>
                        {msg.text}
                    </div>
                </div>
            ))}
        ...
        <div className="mt-4 pt-4 border-t border-border-color dark:border-dark-border-color">
            <div className="flex items-center space-x-2">
                ...
                <button
                    onClick={handleSend}
                    disabled={isLoading}
                    className="bg-primary text-white p-3 rounded-full hover:bg-indigo-700 disabled:opacity-50 transition-colors flex-shrink-0"
                >
